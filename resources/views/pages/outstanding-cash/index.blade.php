          ...
          @forelse($rows as $row)
            <tr>
              <td>
                ...
                <div class="kbsm-business-muted">{{ $row->karyawan?->nama ?? '-' }} &bull; {{ ucfirst($row->karyawan?->status_kerja ?? '-') }}</div>
              </td>
              <td>{{ $row->kelompok }}</td>
              ...
              <td>
                @if(($row->kelompok ?? '') === 'Cicilan Pinjaman')
                  <span class="kbsm-business-muted">
                    {{ $row->siklus_lama ? 'Siklus lama' : 'Tidak eligible payroll' }} &bull;
                    jadwal terbuka {{ $row->jumlah_jadwal_terbuka }} &bull;
