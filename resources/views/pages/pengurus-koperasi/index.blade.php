  ...
  <section id="pengurus-form" class="mb-6 rounded-2xl border border-slate-100 bg-white p-6 shadow-soft-xl {{ (isset($data) || $errors->any()) ? 'block' : 'hidden' }}">
    ...
    <form method="POST" action="{{ isset($data) ? route('pengurus-koperasi.update', $data) : route('pengurus-koperasi.store') }}">
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div>
          <label class="mb-2 block text-xs font-bold uppercase text-slate-600" for="anggota_id">Anggota aktif</label>
        ...
        <div>
          <label class="mb-2 block text-xs font-bold uppercase text-slate-600" for="jabatan">Jabatan organisasi</label>
          <select id="jabatan" name="jabatan" required class="kbsm-focus w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm">
            ...
            @foreach($jabatan as $item)<option value="{{ $item }}" {{ old('jabatan', $data->jabatan ?? '') === $item ? 'selected' : '' }}>{{ $item }}</option>@endforeach
          </select>
        </div>
      </div>
      ...
      <table class="w-full min-w-[850px] text-left text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          @forelse($pengurusKoperasi as $item)
            <tr class="hover:bg-slate-50"><td class="px-6 py-4 font-bold text-[#073b5c]">{{ $item->anggota->nomor_anggota }}</td><td class="px-6 py-4"><div class="font-semibold text-slate-700">{{ $item->anggota->karyawan->nama }}</div><div class="text-xs text-slate-400">{{ $item->anggota->karyawan->jabatan }}</div></td><td class="px-6 py-4 font-semibold text-slate-600">{{ $item->jabatan }}</td><td class="px-6 py-4"><span class="rounded-full px-3 py-1 text-xs font-bold {{ $item->status === 'aktif' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600' }}">{{ ucfirst($item->status) }}</span></td><td class="px-6 py-4"><div class="flex justify-center gap-2"><a href="{{ route('pengurus-koperasi.edit', $item) }}" class="rounded-lg bg-[#073b5c] px-3 py-2 text-xs font-bold text-white">Edit</a>@if($item->status === 'aktif')<form method="POST" action="{{ route('pengurus-koperasi.deactivate', $item) }}">@csrf @method('PATCH')<button class="rounded-lg border border-amber-300 px-3 py-2 text-xs font-bold text-amber-700">Nonaktifkan</button></form>@elseif($item->anggota->status === 'aktif' && $item->anggota->karyawan->status_kerja === 'aktif')<form method="POST" action="{{ route('pengurus-koperasi.activate', $item) }}">@csrf @method('PATCH')<button class="rounded-lg bg-[#2f8f3a] px-3 py-2 text-xs font-bold text-white">Aktifkan</button></form>@endif</div></td></tr>
          @empty
            <tr><td colspan="5" class="px-6 py-10 text-center text-slate-400">Belum ada data Pengurus.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
